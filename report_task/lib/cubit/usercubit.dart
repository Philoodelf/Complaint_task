...
class UserCubit extends Cubit<UserState> {
  ...
  postIt(dynamic result, dynamic selectedCategory) async {
    try {
  ...
  FormData formData = FormData.fromMap({
    ...
    ApiKeys.file: await MultipartFile.fromFile(file.path, filename: fileName), // ✅ File upload
    ApiKeys.category: selectedCategory, // ✅ Ensure you have a selectedCategory variable
  });
  
  var response = await api.post(EndPoints.postComplaint, data: formData);
  if (response.statusCode == 200) {
    ...
    print("⚠️ No file selected. Submitting without a file...");
    ...
    FormData formData = FormData.fromMap({
      ...
      ApiKeys.email: emailcontoller.text,
      ApiKeys.phone: numbercontoller.text,
      ApiKeys.category: selectedCategory ?? "Default",
    });
  
    var response = await api.post(EndPoints.postComplaint, data: formData);
  
    if (response.statusCode == 200) {
      print("✅ Complaint submitted successfully without a file!");
      emit(SendSuccess());
    } else {
      print("⚠️ Failed to submit complaint: ${response.statusMessage}");
    }
